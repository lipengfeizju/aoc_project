...
void imgCallback(const sensor_msgs::ImageConstPtr &msg,
  ...
  cv::resize(cv_ptr->image, cv_ptr->image, cv::Size(0, 0), pr_.resize_val,
             pr_.resize_val);
  ...
  for (int i = 0; i < detections.size(); ++i) {
    ...
    // msg_out.left.push_back( detections[i].left() + 1 ); // make index 1-based
    // instead of 0-based
    // msg_out.top.push_back( detections[i].top() + 1 );
    // msg_out.right.push_back( detections[i].right() + 1 );
    // msg_out.bottom.push_back( detections[i].bottom() + 1 );
    msg_out.left.push_back(
