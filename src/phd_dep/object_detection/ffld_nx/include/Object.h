...
namespace FFLD
{
static const char* objectNameText_[22] =
{
	"aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow",
	"diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa",
	"train", "tvmonitor", "polaris", "unknown"
};
static const char* objectPoseText_[5] =
{
...
public:
	...
	enum Name
	{
		AEROPLANE = 0, BICYCLE, BIRD, BOAT, BOTTLE, BUS, CAR, CAT, CHAIR, COW, DININGTABLE, DOG,
		HORSE, MOTORBIKE, PERSON, POTTEDPLANT, SHEEP, SOFA, TRAIN, TVMONITOR, POLARIS, UNKNOWN
	};
	...
	// Accessors for Object and Pose names
	//inline static void NameToString(Name n, std::string s)
	//{
	//  s = std::string(objectNameText_[n]);
	//}
  inline static Name StringToName( const char* s )
  {
    int num_names = sizeof(objectNameText_)/sizeof(objectNameText_[0]);
    const char ** iter = std::find(objectNameText_, objectNameText_ + num_names, s);
		if (iter != objectNameText_ + num_names)
		  return static_cast<Object::Name>(iter - objectNameText_);
		return Object::UNKNOWN;
  }
	//inline static void PoseToString(Pose p, std::string s)
	//{
	//  s = std::string(objectPoseText_[p]);
	//}
  inline static Pose StringToPose( const char* s )
  {
    int num_poses = sizeof(objectPoseText_)/sizeof(objectPoseText_[0]);
    const char **iter = std::find(objectPoseText_, objectPoseText_ + num_poses, s);
		if (iter != objectPoseText_ + num_poses)
		  return static_cast<Object::Pose>(iter - objectPoseText_);
